...
$query="INSERT INTO  st_SearchedAddresses (sStreetAddress,sCity,sState,sZipCode,sCountry) VALUES ('".$sStreetAddress."','".$locality."','".$administrative_area_level_1."','".$postal_code."','".$country."')";
...
function woocommerce_redirect() {
	...
	if ( is_checkout() && ! empty( $wp->query_vars['order-received'] ) ) {
		wp_redirect( 'https://hugtaste.com' );
		exit;
	}
}
*/
...
add_shortcode( 'exclusivo', 'subscriber' );
...
function do_subscriptions_shortcode($user_id = 1, $product_id = 207, $status = 'any'){
     $subscriptions = wcs_get_users_subscriptions($user_id);
    $has_subscription = false;
    if (empty($product_id)) {
        // Any subscription
        if (!empty($status) && 'any' != $status) {
            // We need to check for a specific status
...
//add_shortcode( 'woocommerce_subscrip_list', 'do_subscriptions_shortcode' );


add_shortcode( 'woocommerce_subscrip_list', 'create_test_sub' );
    ...
    $parent_product = wc_get_product(205);

    $args = array(
        'attribute_frequency' => '3 Days'
    );

    $product_variation = $parent_product->find_matching_product_variation($args);
