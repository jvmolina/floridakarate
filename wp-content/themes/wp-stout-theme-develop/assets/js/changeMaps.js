	...
	bermudaTriangle.setMap(map);
     ...
     function geocodeAddress(ber, geocoder, resultsMap) {
		...
		if (google.maps.geometry.poly.containsLocation(results[0].geometry.location, ber)) {
		    ...
		    idUser=$("#idUserss").val();
		    idSubscription=$("#idSubscriptionss").val();
			//alert(idUser+"--"+idSubscription);
		    var parametros = {
		...
		alert(idSubscription);
		    $.ajax({
				 ...
				 success: function(data) { 
					 $("#myModal2").modal('show');
					setTimeout(location.href='http://localhost/wordpress/?page_id=374&view-subscription='+idSubscription, 1000);
					 }
 			});
		  } else {
