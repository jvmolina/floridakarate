...
<div class="woocommerce_account_subscriptions">
	...
	<?php foreach ( $subscriptions as $subscription_id => $subscription ) : ?>
		<tr class="order">
			...
			<td class="subscription-actions order-actions">
				<a href="<?php echo esc_url( $subscription->get_view_order_url() ) ?>" class="button view"><?php echo esc_html_x( 'View', 'view a subscription', 'woocommerce-subscriptions' ); ?>sssfffffffss</a>
	...
	<?php else : ?>
		...
		<p class="no_subscriptions">
			...
			printf( esc_html__( 'You have no active subscriptions. Find your first subscription in the %sstore%s.', 'woocommerce-subscriptions' ), '<a href="http://localhost/wordpress/?page_id=219">', '</a>' );
			?>
		</p>
